...
const App = () => {
  ...
  return (<Router>
    <Routes>
      ...
      <Route path="/clients" element={isAuthenticated()
          ? (<Layout>
            <Clients/>
          </Layout>)
          : (<Navigate to="/"/>)}/>
      ...
      <Route path="/exchange-rate" element={isAuthenticated()
          ? (<Layout>
            <ExchangeRate/>
          </Layout>)
          : (<Navigate to="/"/>)}/>

      <Route path="/suppliers" element={isAuthenticated()
          ? (<Layout>
            <Suppliers/>
          </Layout>)
          : (<Navigate to="/"/>)}/>
      ...
      <Route path="/orders/print/:id" element={isAuthenticated()
          ? <OrderPrint/>
          : <Navigate to="/"/>}/>

      <Route path="/sales" element={<Layout > <Sales/>
          {/* Section pour la gestion des ventes */
        }
        </Layout>
}/>

      <Route path="/sales/print/:id" element={isAuthenticated()
          ? <SalesPrint/>
          : <Navigate to="/"/>}/>

      <Route path="/sales/payments/:saleId" element={isAuthenticated()
          ? (<Layout>
            <SalePayments/>
          </Layout>)
          : (<Navigate to="/"/>)}/>

      <Route path="/sales/returns/:saleId" element={isAuthenticated()
          ? (<Layout>
            <SaleReturns/>
          </Layout>)
          : (<Navigate to="/"/>)}/>
      <Route path="/sales/refunds/:saleId" element={isAuthenticated()
          ? (<Layout>
            <SaleRefunds/>
          </Layout>)
          : (<Navigate to="/"/>)}/>

      <Route path="/returns/print/:id" element={isAuthenticated()
          ? <ReturnPrint/>
          : <Navigate to="/"/>}/>

      <Route path="/stock-report" element={isAuthenticated()
          ? (<StockReport/>)
          : (<Navigate to="/"/>)}/> {/* Ajouter d'autres routes ici si nécessaire */
      }
      </Routes>
  </Router>);
};
